...
for nnni in range(len(test_set)):
    ...
    for i in range(len(recog_list)):
        ...
        output_line = f"query index: {recog_list[i][0][0]}, matched index: {recog_list[i][0][1]}, error: {t_error_temp}\n"
        print(output_line)
        with open(file_path, "a") as file:  # 使用"a"模式表示追加写入
