...
class MambaVisionMixer(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state],
                               dim=-1)  # 线性变换和拆分：通过 x_proj 和 dt_proj 进行线性变换，并将输出拆分为 dt、B、C 三部分。
...
class ResidualBlock(nn.Module):
    def __init__(self, block, dim):
        ...
        if dim != self.block.downsample(dim):
            self.downsample = nn.Conv2d(dim, self.block.downsample(dim), 1, 2)
    ...
    def forward(self, x):
        ...
        identity = torch.nn.functional.interpolate(identity, size=out.shape[2:], mode="nearest")
        out += identity
        return out
...
class MambaVision(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(len(depths)):
            ...
            self.levels.append(ResidualBlock(level, int(dim * 2 ** i)))
